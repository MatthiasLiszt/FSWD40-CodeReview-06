...
$(document).ready(function(){
 ...
 $('#showMedia').click(function(){
  ...
  mediaData.map(function(o){if(o!==null)
 ...
 $('#loadLocalStorage').click(function(){var catLoad=localStorage.getItem('MyMediaDataCatalog');
                                         console.log(JSON.stringify(localStorage.getItem('MyMediaDataCatalog')));
                                         if(catLoad===undefined)
                                          {$('#generalOutput').html('<h2>error: could not find your media catalog</h2>');}
                                         else 
                                          {mediaData=JSON.parse(catLoad);
                                           $('#generalOutput').html('<h2>your media catalog has been loaded</h2>');}
                                       });

 $('#saveLocalStorage').click(function(){localStorage.setItem('MyMediaDataCatalog',JSON.stringify(mediaData));
                                         console.log(JSON.stringify(localStorage.getItem('MyMediaDataCatalog')));
...
function eraseMedia(){
 if(mediaMaskActiveElement!==undefined)
  ...
  {$('#generalOutput').html('<h2 color="red">error: no media has been selected</h2>');}
}

function removeUndefinedFromArray(x){
 var content=[];
 x.map(function(e){if(e!==undefined){content.push(e);}
                  });
...
function showByCategory(category){
  ...
  if(content.length<1)
   {$('#generalOutput').html('<h2>there is currently no media in this category</h2>');}
  else
   {$('#generalOutput').html(content);}  
}
